...
# ---------------------------------------------------------------------------
# SIH26117 MRPL Industrial Departmental Roles & Clearance Levels
# ---------------------------------------------------------------------------
DEPARTMENT_ROLES = (
    "PROCESS_ENGINEERING",
    "MECHANICAL_INTEGRITY_NDT",
    "OPERATIONS_TAR",
    "HSE_SAFETY",
...
CLEARANCE_LEVELS = {
    "UNCLASSIFIED": 1,
    "CONFIDENTIAL_INTERNAL": 2,
    "RESTRICTED_ENGINEERING": 3,
    "SECRET_EXECUTIVE": 4,
}
...
DEPARTMENT_PERMISSIONS = {
    "PROCESS_ENGINEERING": {
        ...
        "max_clearance": "RESTRICTED_ENGINEERING",
        "allowed_vaults": ["pids", "heat_mass_balances", "crude_assays", "general_sops"],
    },
    "MECHANICAL_INTEGRITY_NDT": {
        "allowed_tools": {
            "calc_vessel_thickness_api510",
        ...
        "max_clearance": "CONFIDENTIAL_INTERNAL",
        "allowed_vaults": ["ndt_logs", "corrosion_loops", "vessel_registers", "api_standards"],
    },
    "OPERATIONS_TAR": {
        "allowed_tools": {
            "plan_refinery_turnaround",
        ...
        "max_clearance": "CONFIDENTIAL_INTERNAL",
        "allowed_vaults": ["turnaround_schedules", "blind_lists", "permits_jsa"],
    },
    "HSE_SAFETY": {
        "allowed_tools": {
            "inspect_pid_drawing",
        ...
        "max_clearance": "RESTRICTED_ENGINEERING",
        "allowed_vaults": ["hazop_studies", "moc_records", "incident_reports"],
    },
    "EXECUTIVE_MANAGEMENT": {
        "allowed_tools": {
            "optimize_crude_blend",
        ...
        "max_clearance": "SECRET_EXECUTIVE",
        "allowed_vaults": ["board_memos", "capex_proposals", "audit_reports", "general_sops"],
    },
    "GENERAL_OPERATIONS": {
        "allowed_tools": {
            "read_file",
...
class AuthManager:
    ...
    def policy(self) -> dict:
        ...
        return {
            ...
            "department_roles": list(DEPARTMENT_ROLES),
            "clearance_levels": list(CLEARANCE_LEVELS.keys()),
        }
    ...
    def create_user(
        ...
        """Create a new user account with optional industrial department and clearance."""
        ...
        with self._config_lock:
            ...
            dept = (department or ("EXECUTIVE_MANAGEMENT" if is_admin else "GENERAL_OPERATIONS")).upper()
            if dept not in DEPARTMENT_ROLES:
                dept = "GENERAL_OPERATIONS"

            clr = (clearance or ("SECRET_EXECUTIVE" if is_admin else "UNCLASSIFIED")).upper()
            if clr not in CLEARANCE_LEVELS:
                clr = "UNCLASSIFIED"
            ...
            self._save()
        logger.info(f"Created user '{username}' (admin={is_admin}, dept={dept}, clr={clr})")
        return True
        ...
        return [
            {
                ...
                "department": self.get_user_department(u),
                "clearance": self.get_user_clearance(u),
            }
    ...
    def status(self, token: Optional[str]) -> Dict[str, Any]:
        ...
        if authenticated:
            ...
            result["department"] = self.get_user_department(username)
            result["clearance"] = self.get_user_clearance(username)
        return result

    def get_user_department(self, username: Optional[str]) -> str:
        """Return the departmental role assigned to a user."""
        if not username:
            return "GENERAL_OPERATIONS"
        user_rec = self.users.get(username.strip().lower(), {})
        if user_rec.get("is_admin"):
            return user_rec.get("department") or "EXECUTIVE_MANAGEMENT"
        return user_rec.get("department") or "GENERAL_OPERATIONS"

    def get_user_clearance(self, username: Optional[str]) -> str:
        """Return the document clearance level assigned to a user."""
        if not username:
            return "UNCLASSIFIED"
        user_rec = self.users.get(username.strip().lower(), {})
        if user_rec.get("is_admin"):
            return user_rec.get("clearance") or "SECRET_EXECUTIVE"
        return user_rec.get("clearance") or "UNCLASSIFIED"
    ...
    def set_user_department(
        ...
        """Update a user's departmental assignment and clearance."""
        username = username.strip().lower()
        ...
        with self._config_lock:
            if username not in self.users:
                return False
            self._config["users"][username]["department"] = dept
            if clearance:
                clr = clearance.upper()
                if clr in CLEARANCE_LEVELS:
                    self._config["users"][username]["clearance"] = clr
            self._save()
        return True

    def check_department_tool_access(self, username: Optional[str], tool_name: str) -> bool:
        """Verify if a user's department is permitted to execute an industrial tool."""
        if not username:
            return False
        if self.is_admin(username):
            return True
        dept = self.get_user_department(username)
        dept_cfg = DEPARTMENT_PERMISSIONS.get(dept, {})
        allowed_tools = dept_cfg.get("allowed_tools", set())
        ...
        try:
            ...
            get_audit_log().record_event(
                user=username,
                department=dept,
                action="TOOL_RBAC_CHECK",
                details={"tool": tool_name, "granted": allowed},
            )
        except Exception as ex:
            logger.debug("Failed to record tool access audit: %s", ex)
        return allowed

    def check_document_clearance(self, username: Optional[str], doc_classification: str) -> bool:
        """Verify if a user's clearance level permits accessing a document classification."""
        if not username:
            return doc_classification.upper() == "UNCLASSIFIED"
        if self.is_admin(username):
            return True
        user_clr = self.get_user_clearance(username)
        user_rank = CLEARANCE_LEVELS.get(user_clr, 1)
        required_rank = CLEARANCE_LEVELS.get(doc_classification.upper(), 4)
        ...
        try:
            ...
            get_audit_log().record_event(
                ...
                action="DOCUMENT_CLEARANCE_CHECK",
                details={
                    "classification": doc_classification.upper(),
                    "user_clearance": user_clr,
                    "granted": passed,
                },
            )
        except Exception as ex:
            logger.debug("Failed to record doc clearance audit: %s", ex)
        return passed
