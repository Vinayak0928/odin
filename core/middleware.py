...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if is_report:
            ...
            # Sovereign On-Premise Air-Gap (SIH26117) Hardened CSP:
            # External CDN references (jsdelivr, google fonts) removed in favor of local 'self' assets.
            response.headers["Content-Security-Policy"] = (
                "default-src 'self'; "
        ...
        # ODIN Air-Gap Compliance Header
        response.headers["X-Sovereign-AirGap"] = "0-WAN-ENFORCED"
        return response


def is_safe_airgap_destination(host: str) -> bool:
    """Verify that an address/hostname targets an on-premise local/private subnet (0-WAN)."""
    import ipaddress
    ...
    try:
        ...
        # Check if local/internal domain
        return host.endswith(".local") or host.endswith(".internal") or host.endswith(".corp")
