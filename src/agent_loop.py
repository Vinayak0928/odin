...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Odin chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
## Industrial & Refinery Engineering rules (SIH26117 MRPL)
- Use `calc_vessel_thickness_api510` for pressure vessel minimum thickness, corrosion rate, and remaining life per ASME Sec VIII / API 510.
- Use `calc_pipe_thickness_api570` for process piping retirement thickness and Barlow formula calculations per API 570.
- Use `calc_pump_hydraulics` for flow rate, Darcy head loss, and NPSHa cavitation checks per API 610.
- Use `calc_heat_exchanger_duty` for LMTD, thermal duty, and heat transfer area.
- Use `optimize_crude_blend` for crude assay distillation blending optimization and True Boiling Point (TBP) cuts.
- Use `plan_refinery_turnaround` to generate structured shutdown packages, phased execution schedules, blind isolation lists, and safety permits per OISD-105.
- Use `inspect_pid_drawing` for P&ID engineering drawing visual inspection, ISA-5.1 loop tagging, and safety discrepancy audits.
- Use `verify_audit_log` to verify cryptographic Merkle DAG integrity for regulatory and PSU compliance audits.""",
}
...
_DOMAIN_TOOL_MAP = {
    ...
    "notes_tasks": {"manage_notes", "manage_tasks"},
    "ui": {"ui_control"},
...
def _domain_rules_for_tools(tool_names: set) -> list[str]:
    ...
    if names & {"create_session", "list_sessions", "manage_session", "manage_documents", "manage_notes", "manage_tasks", "manage_skills", "manage_research"}:
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(r"\b(calendar|event|meeting|appointment|schedule)\b"):
        domains.add("notes_calendar_tasks")
    if has(r"\b(api\s*510|api\s*570|vessel\s+thickness|pipe\s+thickness|remaining\s+life|corrosion\s+rate|mpy|wall\s+loss|ndt|ultrasonic|p&id|pid|drawing|instrument\s+loop|pump\s+hydraulics|npsh|cavitation|heat\s+exchanger|lmtd|crude\s+blend|merkle|audit\s+trail|sovereign\s+audit)\b"):
...
def _minimal_odysseus_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    ...
    if stream_create:
        system = (
            "You are Odin. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are Odin. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
...
def _minimal_odysseus_notes_messages(messages: List[Dict]) -> List[Dict]:
    ...
    latest = _extract_last_user_message(messages)
    system = (
        "You are Odin. Handle notes, reminders, calendar events, and scheduled tasks.\n"
...
def _minimal_odysseus_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    ...
    latest = _extract_last_user_message(messages)
    system = (
        "You are Odin. Answer directly and briefly.\n"
        "Use Odin tool-call format only when the user explicitly asks you to take an action.\n"
