...
def blocked_tools_for_owner(owner: Optional[str]) -> Set[str]:
    """Tools to hide/disable for this owner under public-user and departmental policy."""
    if owner_is_admin_or_single_user(owner):
        return set()

    blocked = set(NON_ADMIN_BLOCKED_TOOLS)

    # Departmental RBAC integration for MRPL (SIH26117)
    try:
        from core.auth import AuthManager, DEPARTMENT_PERMISSIONS
        auth = AuthManager()
        if auth.is_configured and owner:
            dept = auth.get_user_department(owner)
            dept_cfg = DEPARTMENT_PERMISSIONS.get(dept, {})
            allowed_dept_tools = dept_cfg.get("allowed_tools", set())
            # Unblock departmental allowed tools
            blocked = blocked - allowed_dept_tools

            # Block industrial tools not allowed for this department
            all_industrial_tools = {
            ...
            unauthorized_industrial = all_industrial_tools - allowed_dept_tools
            blocked.update(unauthorized_industrial)
    except Exception as exc:
        logger.warning("Unable to resolve departmental tool access: %s", exc)
