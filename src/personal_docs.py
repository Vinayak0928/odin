...
def extract_pdf_text(file_path: str) -> str:
    """Extract text from a PDF file using pypdf, falling back to local OCR for scanned pages."""
    try:
        ...
        for i, page in enumerate(reader.pages):
            ...
            if len(txt) < 50:
                try:
                    from src.document_processor import _extract_page_text_ocr
                    ocr = _extract_page_text_ocr(file_path, i)
                    if ocr:
