...
async def execute_tool_block(
    ...
    try:
        ...
        try:
            ...
            user = owner or "anonymous"
            dept = "GENERAL_OPERATIONS"
            try:
                dept = AuthManager().get_user_department(user)
            except Exception:
                pass
            tool_name = getattr(block, "tool_type", None) or "unknown"
            if not tool_name.startswith(("calc_", "optimize_crude_", "inspect_pid_", "verify_audit_")):
                get_audit_log().record_event(
                    ...
                    details={
                        "session_id": session_id,
                        "desc": str(output[0])[:120] if output else "",
                        "success": output[1].get("exit_code", 0) == 0 if isinstance(output[1], dict) else True,
                    },
                )
        except Exception:
            pass
