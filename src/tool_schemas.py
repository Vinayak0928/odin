...
FUNCTION_TOOL_SCHEMAS = [
    {
        "type": "function",
        "function": {
            "name": "calc_vessel_thickness_api510",
            "description": "Calculate minimum required thickness (t_min), remaining life, and next inspection interval for refinery pressure vessels per API 510 and ASME Sec VIII Div 1 UG-27.",
            "parameters": {
                "type": "object",
                "properties": {
                    "vessel_tag": {"type": "string", "description": "Equipment tag, e.g. V-101"},
                    "design_pressure_psi": {"type": "number", "description": "Internal design pressure in PSI"},
                    "inside_radius_in": {"type": "number", "description": "Inside shell radius in inches"},
                    "allowable_stress_psi": {"type": "number", "description": "Maximum allowable stress S in PSI"},
                    "joint_efficiency": {"type": "number", "description": "Weld joint efficiency E (0.7 to 1.0)"},
                    "actual_thickness_in": {"type": "number", "description": "Measured ultrasonic wall thickness in inches"},
                    "corrosion_rate_mpy": {"type": "number", "description": "Corrosion rate in mils per year (MPY)"}
                },
                "required": ["design_pressure_psi", "inside_radius_in", "allowable_stress_psi", "joint_efficiency", "actual_thickness_in", "corrosion_rate_mpy"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "calc_pipe_thickness_api570",
            "description": "Calculate minimum required thickness and remaining life for process piping per API 570 and ASME B31.3 Barlow's formula.",
            "parameters": {
                "type": "object",
                "properties": {
                    "pipe_tag": {"type": "string", "description": "Piping line identifier, e.g. 8-HC-101-A1A"},
                    "pipe_class": {"type": "string", "enum": ["Class 1", "Class 2", "Class 3"], "description": "API 570 pipe class"},
                    "design_pressure_psi": {"type": "number", "description": "Design pressure in PSI"},
                    "outside_diameter_in": {"type": "number", "description": "Pipe outer diameter in inches"},
                    "allowable_stress_psi": {"type": "number", "description": "Allowable stress S in PSI"},
                    "joint_efficiency": {"type": "number", "description": "Joint efficiency E (0.8 to 1.0)"},
                    "actual_thickness_in": {"type": "number", "description": "Current measured wall thickness in inches"},
                    "corrosion_rate_mpy": {"type": "number", "description": "Corrosion rate in MPY"}
                },
                "required": ["design_pressure_psi", "outside_diameter_in", "allowable_stress_psi", "joint_efficiency", "actual_thickness_in", "corrosion_rate_mpy"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "calc_pump_hydraulics",
            "description": "Evaluate pump hydraulic performance, Darcy-Weisbach head loss, and NPSH cavitation margin per API 610.",
            "parameters": {
                "type": "object",
                "properties": {
                    "pump_tag": {"type": "string", "description": "Pump equipment tag, e.g. P-102A"},
                    "flow_rate_gpm": {"type": "number", "description": "Process flow rate in US GPM"},
                    "pipe_diameter_in": {"type": "number", "description": "Pipe internal diameter in inches"},
                    "pipe_length_ft": {"type": "number", "description": "Total equivalent pipe length in feet"},
                    "fluid_viscosity_cp": {"type": "number", "description": "Dynamic viscosity in centipoise (cP)"},
                    "fluid_density_lb_cuft": {"type": "number", "description": "Fluid density in lb/cu.ft"},
                    "suction_static_head_ft": {"type": "number", "description": "Static head at pump suction in feet"},
                    "vapor_pressure_psia": {"type": "number", "description": "True vapor pressure in PSIA"},
                    "pump_npsh_required_ft": {"type": "number", "description": "Manufacturer NPSHr in feet"}
                },
                "required": ["flow_rate_gpm", "pipe_diameter_in", "pipe_length_ft", "fluid_viscosity_cp", "fluid_density_lb_cuft", "suction_static_head_ft", "vapor_pressure_psia", "pump_npsh_required_ft"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "calc_heat_exchanger_duty",
            "description": "Calculate heat exchanger thermal duty (Q), Log Mean Temperature Difference (LMTD), and required heat transfer area per TEMA.",
            "parameters": {
                "type": "object",
                "properties": {
                    "exchanger_tag": {"type": "string", "description": "Exchanger tag, e.g. E-101"},
                    "hot_inlet_temp_c": {"type": "number", "description": "Hot stream inlet temp in Celsius"},
                    "hot_outlet_temp_c": {"type": "number", "description": "Hot stream outlet temp in Celsius"},
                    "cold_inlet_temp_c": {"type": "number", "description": "Cold stream inlet temp in Celsius"},
                    "cold_outlet_temp_c": {"type": "number", "description": "Cold stream outlet temp in Celsius"},
                    "hot_mass_flow_kg_s": {"type": "number", "description": "Hot stream mass flow rate in kg/s"},
                    "hot_specific_heat_kj_kg_c": {"type": "number", "description": "Hot stream specific heat Cp in kJ/(kg*C)"},
                    "overall_u_w_m2_c": {"type": "number", "description": "Overall heat transfer coefficient U in W/(m^2*C)"}
                },
                "required": ["hot_inlet_temp_c", "hot_outlet_temp_c", "cold_inlet_temp_c", "cold_outlet_temp_c", "hot_mass_flow_kg_s", "hot_specific_heat_kj_kg_c"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "optimize_crude_blend",
            "description": "Optimize crude oil blending ratios for distillation throughput to meet sulfur and API gravity specifications.",
            "parameters": {
                "type": "object",
                "properties": {
                    "crude_candidates": {
                        "type": "array",
                        "items": {"type": "object"},
                        "description": "List of available crude assays with sulfur_wt and api_gravity"
                    },
                    "target_max_sulfur_wt": {"type": "number", "description": "Maximum blended sulfur limit (wt%)"},
                    "target_min_api_gravity": {"type": "number", "description": "Minimum blended API gravity"},
                    "total_bpd": {"type": "number", "description": "Total crude unit throughput in barrels/day"}
                },
                "required": ["target_max_sulfur_wt", "target_min_api_gravity"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "inspect_pid_drawing",
            "description": "Inspect and parse P&ID drawing text or image for ISA-5.1 instrumentation loops, valves, and safety relief discrepancies.",
            "parameters": {
                "type": "object",
                "properties": {
                    "drawing_text": {"type": "string", "description": "OCR or extracted text from the P&ID drawing"},
                    "image_path": {"type": "string", "description": "Optional local file path to the drawing image for tiled segmentation"}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            ...
            "description": "Verify cryptographic integrity and Merkle root hash of the air-gapped refinery audit trail.",
            "parameters": {
                "type": "object",
                "properties": {
                    "verification_scope": {"type": "string", "description": "Optional scope, e.g. 'all' or 'recent'"}
                }
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "plan_refinery_turnaround",
            "description": "Generate comprehensive refinery turnaround / shutdown execution package with phased schedule, blinding list, and OISD-105 safety controls.",
            "parameters": {
                "type": "object",
                "properties": {
                    "unit_name": {"type": "string", "description": "Refinery unit name, e.g. 'CDU-1', 'CDU-2', 'VDU', 'FCCU', 'HCU', 'DHDS', 'SRU'"},
                    "turnaround_type": {"type": "string", "enum": ["MAJOR_REVAMP", "CATALYST_CHANGE", "MANDATORY_STATUTORY_INSPECTION", "MINI_SHUTDOWN"], "description": "Type of turnaround"},
                    "target_duration_days": {"type": "integer", "description": "Target shutdown window in days (typically 21-45 days)"},
                    "include_blind_list": {"type": "boolean", "description": "Whether to include battery-limit positive isolation blind list"}
                },
                "required": ["unit_name"]
            }
        }
