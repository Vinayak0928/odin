...
def _get_rapid_ocr():
    """Lazy-load and cache RapidOCR engine."""
    ...
    if _rapid_ocr_engine is None:
        try:
            ...
            logger.debug(f"RapidOCR engine initialization failed: {e}")
            _rapid_ocr_engine = False
    return _rapid_ocr_engine if _rapid_ocr_engine is not False else None


def _extract_page_text_ocr(pdf_path: str, page_index: int) -> str:
    """Render a PDF page to image and perform local OCR using RapidOCR (zero VRAM, fast CPU)."""
    engine = _get_rapid_ocr()
    if engine is None:
        return ""
    try:
        import numpy as np
        # Try fitz first if available
        try:
            import fitz
            ...
            try:
                if page_index < doc.page_count:
                    ...
                    ocr_res, _ = engine(img_arr)
                    if ocr_res:
                        return "\n".join(item[1].strip() for item in ocr_res if item[1] and item[1].strip())
            finally:
                doc.close()
        except Exception:
            pass
        ...
        try:
            ...
            if page_index < len(doc):
                bitmap = doc[page_index].render(scale=2.0)
                ocr_res, _ = engine(np.array(bitmap.to_pil()))
                if ocr_res:
                    return "\n".join(item[1].strip() for item in ocr_res if item[1] and item[1].strip())
        except Exception as e:
            logger.debug(f"pypdfium2 OCR render failed: {e}")
    except Exception as e:
        logger.warning(f"OCR extraction error for {pdf_path} page {page_index}: {e}")
    return ""


def _process_pdf(path: str, owner: str | None = None) -> str:
    """Process PDF file with text extraction (pypdf) and local OCR for scanned/drawing pages."""
    try:
        ...
        vl_available = bool(settings.get("vision_enabled", False) and settings.get("vision_model"))

        for page_num, page in enumerate(reader.pages):
            page_text = (page.extract_text() or "").strip()

            # For pages with very little text (e.g. scanned pages, CAD/vector outlines), use local OCR
            if len(page_text) < 50:
                ocr_text = _extract_page_text_ocr(path, page_num)
                if ocr_text:
                    page_text = (page_text + "\n" + ocr_text).strip() if page_text else ocr_text

            # If still minimal text and vision model is configured, try VL model
            if len(page_text) < 50 and vl_available:
                try:
                    ...
                    for img_index, img in enumerate(images[:3]):
                        try:
                            ...
                            try:
                                ...
                                vl_text = analyze_image_with_vl(temp_img_path, owner=owner)
                                if vl_text and not any(k in vl_text.lower() for k in ("unavailable", "not configured", "disabled")):
                                    page_text += f"\n[Image {img_index + 1} analysis]: {vl_text}"
                            finally:
                                try:
        ...
        if pdf_text.strip():
            # Industrial P&ID / NDT Log enrichment
            filename_lower = os.path.basename(path).lower()
            if any(k in filename_lower for k in ("pid", "p&id", "drawing", "ndt", "thickness", "isometric", "schematic")):
                try:
                    from src.tools.pid_inspector import parse_pid_entities_from_text, audit_pid_safety_discrepancies
                    parsed = parse_pid_entities_from_text(pdf_text)
                    if parsed.get("instruments") or parsed.get("equipment") or parsed.get("piping_lines"):
                        discrepancies = audit_pid_safety_discrepancies(parsed)
                        enrichment = f"\n\n=== [ODIN Industrial Drawing Summary] ===\n"
                        enrichment += f"Equipment Identified: {len(parsed.get('equipment', []))} units\n"
                        enrichment += f"Instrumentation Loops: {len(parsed.get('instruments', []))} loops\n"
                        enrichment += f"Piping Lines: {len(parsed.get('piping_lines', []))} lines\n"
                        if discrepancies:
                            enrichment += f"Safety Discrepancies / HAZOP Alerts: {len(discrepancies)} flagged\n"
                        pdf_text = enrichment + pdf_text
                except Exception as ex:
                    logger.debug(f"P&ID entity extraction skipped for {path}: {ex}")
        ...
        if upload_handler.is_image_file(display_name, mime):
            ...
            if mime == "application/pdf":
                    ...
                    try:
                        ...
                        if is_form:
                            ...
                            if doc_id:
                                ...
                                if body_for_chat and "no readable content found" not in body_for_chat.lower():
                                    extracted_text += (
                                        f"\n\n--- ATTACHED DOCUMENT CONTENT ({title}) ---\n"
                                        f"[The user has attached form '{title}'. The extracted text is provided below. "
                                        f"Analyze this content to answer any questions]:\n\n"
                                        f"{body_for_chat}{truncated_marker}\n"
                            ...
                            if doc_id:
                                if body_for_chat and "no readable content found" not in body_for_chat.lower():
                                    extracted_text = (
                                        f"\n\n--- ATTACHED DOCUMENT CONTENT ({title}) ---\n"
                                        f"[The user has attached the document '{title}'. The extracted text is provided below. "
                                        f"Analyze this content to answer the user's request]:\n\n"
                                        f"{body_for_chat}{truncated_marker}\n"
                                        f"--- END OF ATTACHED DOCUMENT ---\n"
                                    )
                                else:
                                    extracted_text = (
                                        f"\n\n[Attached document: {title} — opened in document viewer. "
                                        f"No readable text or characters found.]"
                                    )
                ...
                if extracted_text is None:
                    raw_extracted = _process_pdf(path, owner=owner)
                    stripped = strip_pdf_content_marker(raw_extracted)
                    if stripped and "no readable content found" not in stripped.lower():
                        title = os.path.splitext(os.path.basename(display_name))[0]
                        extracted_text = (
                            f"\n\n--- ATTACHED DOCUMENT CONTENT ({title}) ---\n"
                            f"[The user has attached the document '{title}'. The extracted text is provided below. "
