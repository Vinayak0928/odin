...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
        ...
        if not await asyncio.to_thread(auth_manager.verify_password, username, body.password):
            try:
                ...
                get_audit_log().record_event(
                    ...
                    action="LOGIN_FAILED",
                    details={"ip": request.client.host if request.client else "unknown", "reason": "invalid_password"},
                )
            except Exception:
                pass
        ...
        if auth_manager.totp_enabled(username):
            ...
            if not auth_manager.totp_verify(username, body.totp_code):
                try:
                    ...
                    get_audit_log().record_event(
                        user=username,
                        department=auth_manager.get_user_department(username),
                        action="LOGIN_FAILED",
    ...
    async def logout(request: Request, response: Response):
        ...
        if user:
            try:
                ...
                get_audit_log().record_event(
                    user=user,
                    department=auth_manager.get_user_department(user),
                    action="USER_LOGOUT",
                    details={},
                )
            except Exception:
    ...
    @router.put("/users/{username}/department")
    async def update_user_department(username: str, body: UpdateDepartmentRequest, request: Request):
        user = _get_current_user(request)
        if not user or not auth_manager.is_admin(user):
            raise HTTPException(403, "Admin only")
        ok = auth_manager.set_user_department(username, body.department, body.clearance)
        if not ok:
            raise HTTPException(400, "Invalid department or user not found")
        try:
            from core.audit_merkle import get_audit_log
            get_audit_log().record_event(
                user=user,
        ...
        return {
            ...
            "username": username,
            "department": auth_manager.get_user_department(username),
            "clearance": auth_manager.get_user_clearance(username),
        }
