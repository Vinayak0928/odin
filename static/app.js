...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'ODIN Chat';
  ...
  // MRPL P&ID Inspector tool button
  const toolPidBtn = el('tool-pid-btn');
  if (toolPidBtn) {
    toolPidBtn.addEventListener('click', async () => {
      const { openPIDInspector } = await import('./js/industrial.js');
      openPIDInspector();
    });
  }

  // MRPL Merkle Audit Trail tool button
  const toolAuditBtn = el('tool-audit-btn');
  if (toolAuditBtn) {
    toolAuditBtn.addEventListener('click', async () => {
      const { openAuditTrail } = await import('./js/industrial.js');
      openAuditTrail();
    });
  }
  ...
  const _routeOpen = {
    ...
    '/pid':      () => document.getElementById('tool-pid-btn')?.click(),
    '/audit':    () => document.getElementById('tool-audit-btn')?.click(),
  ...
  fetch(`${API_BASE}/api/auth/status`, { credentials: 'same-origin' })
    ...
    .then(d => {
      ...
      if (d.department) {
        window._userDepartment = d.department;
        const deptLabel = document.getElementById('mrpl-dept-label');
        if (deptLabel) {
          deptLabel.textContent = d.department.replace(/_/g, ' ');
        }
        const deptBadge = document.getElementById('mrpl-dept-badge');
        if (deptBadge) {
          deptBadge.title = `Active Refinery Department: ${d.department} | Clearance: ${d.clearance || 'UNCLASSIFIED'}`;
        }
      }
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  {
    const st = loadToggleState();
    const ragState = st.rag !== undefined ? st.rag : true;
  ...
  (function initModelPickerResponsive() {
    ...
    const TOOLBAR_HIDE_WIDTH = 160;
	    ...
	    function setComposerPlaceholder(width) {
	      ...
	      textarea.setAttribute('placeholder', width < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message ODIN...');
	    }
...
function startOdysseusApp() {
  ...
  const _railToolMap = {
    'rail-pid':       'tool-pid-btn',
    'rail-audit':     'tool-audit-btn',
  ...
  // ── Merkle DAG ODIN Audit Trail Modal ──
  window._openMerkleAuditModal = async function() {
    const modal = document.getElementById('merkle-audit-modal');
    if (!modal) return;
    modal.classList.remove('hidden');
    modal.style.display = 'flex';
    await window._loadMerkleAuditData();
  };

  window._closeMerkleAuditModal = function() {
    const modal = document.getElementById('merkle-audit-modal');
    if (!modal) return;
    modal.classList.add('hidden');
  ...
  window._loadMerkleAuditData = async function() {
    try {
      const statsRes = await fetch(`${API_BASE}/api/audit/stats`, { credentials: 'same-origin' });
      if (statsRes.ok) {
        const stats = await statsRes.json();
        const egressEl = document.getElementById('merkle-modal-egress');
        const compEl = document.getElementById('merkle-modal-compliance');
        const countEl = document.getElementById('merkle-modal-block-count');
        const rootEl = document.getElementById('merkle-modal-root');
        if (egressEl) egressEl.textContent = stats.egress_status || '0-WAN VERIFIED INTERNAL';
        if (compEl) {
          compEl.textContent = stats.compliance_status || (stats.is_valid ? 'CERTIFIED TAMPER-EVIDENT' : 'TAMPERING DETECTED');
          compEl.style.color = stats.is_valid ? '#50fa7b' : '#ff5555';
        }
        if (countEl) countEl.textContent = stats.total_blocks || 0;
        if (rootEl) rootEl.textContent = stats.merkle_root || '0'.repeat(64);
      }

      const blocksRes = await fetch(`${API_BASE}/api/audit/blocks?limit=50`, { credentials: 'same-origin' });
      if (blocksRes.ok) {
        const data = await blocksRes.json();
        const tbody = document.getElementById('merkle-table-body');
        if (tbody) {
          if (!data.blocks || data.blocks.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="padding:16px;text-align:center;color:#888;">No audit blocks recorded yet. Actions in the workbench will appear here automatically.</td></tr>';
          } else {
            tbody.innerHTML = data.blocks.slice().reverse().map(b => `
              <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                <td style="padding: 6px 8px; font-family: monospace; font-weight: 600; color: #61afef;">#${b.index}</td>
                <td style="padding: 6px 8px; color: var(--color-muted, #888); white-space: nowrap;">${(b.timestamp || '').replace('T', ' ').slice(0, 19)}</td>
                <td style="padding: 6px 8px;"><span style="font-weight:600;">${b.user || 'anon'}</span> <span style="font-size:10px;opacity:0.7;">(${b.department || 'GENERAL'})</span></td>
                <td style="padding: 6px 8px; font-family: monospace; color: #e5c07b;">${b.action || ''}</td>
                <td style="padding: 6px 8px; font-family: monospace; font-size: 10px; color: #50fa7b;" title="${b.hash || ''}">${(b.hash || '').slice(0, 12)}...${(b.hash || '').slice(-8)}</td>
                <td style="padding: 6px 8px; font-family: monospace; font-size: 10px; color: var(--color-muted, #888);" title="${b.prev_hash || ''}">${(b.prev_hash || '').slice(0, 8)}...</td>
              </tr>
            `).join('');
          }
        }
      ...
      console.error('Failed to load Merkle audit data:', e);
    }
  };

  window._verifyMerkleChainLive = async function() {
    const msgEl = document.getElementById('merkle-verify-msg');
    const verifyBtn = document.getElementById('merkle-btn-verify');
    if (verifyBtn) verifyBtn.disabled = true;
    if (msgEl) {
      msgEl.style.display = 'block';
      msgEl.style.background = 'rgba(97, 175, 239, 0.15)';
      msgEl.style.color = '#61afef';
      msgEl.textContent = 'Verifying SHA-256 Merkle DAG chain integrity...';
    }
    try {
      const res = await fetch(`${API_BASE}/api/audit/verify`, { credentials: 'same-origin' });
      const data = await res.json();
      if (msgEl) {
        if (data.is_valid) {
          msgEl.style.background = 'rgba(80, 250, 123, 0.15)';
          msgEl.style.color = '#50fa7b';
          msgEl.style.border = '1px solid rgba(80, 250, 123, 0.4)';
          msgEl.textContent = `✔ Chain Verified! All ${data.total_blocks_verified} block(s) cryptographically valid. Root: ${data.merkle_root.slice(0, 16)}... (0-WAN Internal)`;
        } else {
          msgEl.style.background = 'rgba(255, 85, 85, 0.15)';
          msgEl.style.color = '#ff5555';
          msgEl.style.border = '1px solid rgba(255, 85, 85, 0.4)';
          msgEl.textContent = `✖ Tampering Detected! ${data.discrepancies.length} discrepancy(ies) flagged.`;
        }
      }
      await window._loadMerkleAuditData();
    } catch (e) {
      if (msgEl) {
        msgEl.style.background = 'rgba(255, 85, 85, 0.15)';
        msgEl.style.color = '#ff5555';
        msgEl.textContent = 'Verification request failed: ' + e.message;
      }
    } finally {
  ...
  const closeMerkleBtn = document.getElementById('close-merkle-modal');
  if (closeMerkleBtn) closeMerkleBtn.addEventListener('click', window._closeMerkleAuditModal);
  const verifyMerkleBtn = document.getElementById('merkle-btn-verify');
  if (verifyMerkleBtn) verifyMerkleBtn.addEventListener('click', window._verifyMerkleChainLive);
  const refreshMerkleBtn = document.getElementById('merkle-btn-refresh');
  if (refreshMerkleBtn) refreshMerkleBtn.addEventListener('click', window._loadMerkleAuditData);
  const merkleModal = document.getElementById('merkle-audit-modal');
  if (merkleModal) {
    merkleModal.addEventListener('click', (e) => {
      if (e.target === merkleModal) window._closeMerkleAuditModal();
    });
  }
}
