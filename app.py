...
app = FastAPI(
    title="ODIN (SIH26117)",
    description="ODIN (Mangalore Refinery & Petrochemicals Limited) Confidential Industrial AI Workbench",
    version="2.0.0",
)
...
# Audit (Merkle DAG cryptographic compliance trail)
from routes.audit_routes import setup_audit_routes
audit_router = setup_audit_routes(auth_manager)
...
# Codex integration — HTTP surface for the Codex plugin/MCP bridge.
...
# ODIN Industrial & Merkle Audit routes (SIH26117)
from routes.audit_routes import setup_audit_routes
app.include_router(setup_audit_routes(auth_manager))

from routes.industrial_routes import setup_industrial_routes
app.include_router(setup_industrial_routes())
logger.info("ODIN industrial calculations and Merkle audit routes initialized")
...
@app.get("/pid")
async def serve_pid(request: Request):
    return await serve_index(request)

@app.get("/audit")
async def serve_audit(request: Request):
    return await serve_index(request)
...
async def _startup_event():
    ...
    # Auto-register local Ollama endpoint for ODIN air-gapped models if detected
    async def _auto_seed_local_models():
        try:
            import json as _json, httpx as _httpx
            from core.database import SessionLocal as _SL, ModelEndpoint as _ME
            from src.settings import load_settings as _ls, save_settings as _ss
                        ...
                        if m_list:
                            ...
                            try:
                                ...
                                if not ep:
                                    ep = _ME(
                                        id="ollama-local",
                                        name="Local Ollama (ODIN LLM)",
                                        base_url="http://localhost:11434/v1",
                                        is_enabled=True,
                                        model_type="llm",
                                        endpoint_kind="local",
                                        model_refresh_mode="auto",
                                        cached_models=_json.dumps(m_list),
                                        supports_tools=False,
                                        owner=None,
                                    )
                                    db.add(ep)
                                    db.commit()
                                    logger.info("Auto-registered local Ollama endpoint with models: %s", m_list)
                                else:
                                    ep.cached_models = _json.dumps(m_list)
                                    db.commit()
            ...
            logger.debug("Local model auto-seed skipped: %s", ex)

    _startup_tasks.append(asyncio.create_task(_auto_seed_local_models()))
