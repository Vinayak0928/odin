...
if ($pyLauncher) {
    foreach ($v in @("-3.14", "-3.13", "-3.12", "-3.11", "")) {
        $argsList = if ($v) { @($v) } else { @() }
        $ver = Get-PythonVersionText $pyLauncher.Source $argsList
        if ($ver) {
...
if (-not $pyExe) {
    Fail "Couldn't find Python 3.11+ for Windows setup. Install Python 3.11+ (or open the Python launcher with 'py') from https://www.python.org/downloads/, then re-run this script."
}
...
# 7. Port check & graceful cleanup of previous run
$existingConn = Get-NetTCPConnection -LocalPort $Port -ErrorAction SilentlyContinue | Where-Object { $_.State -eq "Listen" }
if ($existingConn) {
    Write-Host ("Port {0} is currently occupied by PID {1}. Releasing port..." -f $Port, $existingConn.OwningProcess) -ForegroundColor Yellow
...
$env:APP_PORT = $Port
Write-Step ("Starting MRPL Sovereign AI Workbench at http://{0}:{1}" -f $BindHost, $Port)
